...
type
  TfExemplo = class(TForm)
    ...
    procedure BitBtnSaldoClick(Sender: TObject);
  private
...
uses
  Gateway.Saque, Gateway.Deposito, Gateway.Saldo, Connection.DB;
...
procedure TfExemplo.BitBtnSaldoClick(Sender: TObject);
var
  SaldoGateway: TSaldoGateway;
begin
  SaldoGateway := TSaldoGateway.Create(TDBConnection.Create);
  try
    ShowMessage(SaldoGateway.RetornarSaldo.ToString);
  finally
    SaldoGateway.Free;
  end;
end;
